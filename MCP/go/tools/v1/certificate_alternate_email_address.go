...
func Certificate_alternate_email_addressHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/certificates/%s/email/resend/%s", cfg.BaseURL, certificateId, emailAddress)
...
func CreateCertificate_alternate_email_addressTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1_certificates_certificateId_email_resend_emailAddress",
		mcp.WithDescription("Add alternate email address"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Certificate_alternate_email_addressHandler(cfg),
	}
}
