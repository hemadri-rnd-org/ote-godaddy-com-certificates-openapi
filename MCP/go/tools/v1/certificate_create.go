...
func Certificate_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCertificate_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1_certificates",
		mcp.WithDescription("Create a pending order for certificate"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Certificate_createHandler(cfg),
	}
}
