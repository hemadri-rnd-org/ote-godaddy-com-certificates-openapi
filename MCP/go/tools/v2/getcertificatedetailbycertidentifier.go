...
func GetcertificatedetailbycertidentifierHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/certificates/%s", cfg.BaseURL, customerId, certificateId)
		...
		var result models.CertificateDetailV2
...
func CreateGetcertificatedetailbycertidentifierTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_customers_customerId_certificates_certificateId",
		mcp.WithDescription("Retrieve individual certificate details"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetcertificatedetailbycertidentifierHandler(cfg),
	}
}
